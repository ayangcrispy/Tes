...
function openTab(tabId) {
	// Logika untuk membuka tab berdasarkan tabId
	// Anda mungkin sudah memiliki logika ini di fungsi openTab yang ada.
...
const upload1 = document.getElementById('upload-all');
...
function cropAndDisplayImages1() {
	const cropPositions = [{
		...
		{
			xOffset: (canvas1.width - (canvas1.height * 4) / 5) / 2,
			buttonIndex: 1
		}, // Crop tengah atas
		{
			xOffset: canvas1.width - (canvas1.height * 4) / 5,
			buttonIndex: 2
		} // Crop kanan atas
	];

	cropPositions.forEach(({
		xOffset,
		buttonIndex
	}) => {
...
const upload2 = document.getElementById('upload-all');
...
function cropAndDisplayImages2() {
	const cropPositions = [{
		...
		{
			yOffset: 0,
			xOffset: (canvas2.width - (canvas2.height / 2 * 4) / 5) / 2,
			buttonIndex: 1
		}, // Crop tengah atas
		{
			yOffset: 0,
			xOffset: canvas2.width - (canvas2.height / 2 * 4) / 5,
			buttonIndex: 2
		}, // Crop kanan atas
		{
			yOffset: (canvas2.height / 2),
			xOffset: 0,
			buttonIndex: 3
		}, // Crop kiri bawah 
		{
...
const upload3 = document.getElementById('upload-all');
...
function cropAndDisplayImages3() {
	const cropPositions = [{
		...
		{
			yOffset: 0,
			xOffset: (canvas3.width - (canvas3.height / 3 * 4) / 5) / 2,
			buttonIndex: 1
		}, // Crop tengah atas
		{
			yOffset: 0,
			xOffset: canvas3.width - (canvas3.height / 3 * 4) / 5,
			buttonIndex: 2
		}, // Crop kanan atas

		{
			yOffset: (canvas3.height / 3),
			xOffset: 0,
			buttonIndex: 3
		}, // Crop kiri tengah atas
		{
			yOffset: (canvas3.height / 3),
			xOffset: (canvas3.width - (canvas3.height / 3 * 4) / 5) / 2,
			buttonIndex: 4
		}, // Crop tengah tengah atas
		{
			yOffset: (canvas3.height / 3),
			xOffset: canvas3.width - (canvas3.height / 3 * 4) / 5,
			buttonIndex: 5
		}, // Crop kanan tengah atas

		{
			yOffset: (canvas3.height * (2 / 3)),
			xOffset: 0,
			buttonIndex: 6
		}, // Crop kiri tengah bawah 
		{
			yOffset: (canvas3.height * (2 / 3)),
			xOffset: (canvas3.width - (canvas3.height / 3 * 4) / 5) / 2,
			buttonIndex: 7
...
const upload4 = document.getElementById('upload-all');
...
function cropAndDisplayImages4() {
	const cropPositions = [{
		...
		{
			yOffset: 0,
			xOffset: (canvas4.width - (canvas4.height / 4 * 4) / 5) / 2,
			buttonIndex: 1
		}, // Crop tengah atas
		{
			yOffset: 0,
			xOffset: canvas4.width - (canvas4.height / 4 * 4) / 5,
			buttonIndex: 2
		}, // Crop kanan atas

		{
			yOffset: (canvas4.height / 4),
			xOffset: 0,
			buttonIndex: 3
		}, // Crop kiri tengah atas
		{
		...
		}, // Crop tengah bawah 
		{
			...
			xOffset: canvas4.width - (canvas4.height / 4 * 4) / 5,
			buttonIndex: 8
		}, // Crop kanan tengah bawah

		{
			yOffset: (canvas4.height * (3 / 4)),
			xOffset: 0,
			buttonIndex: 9
		}, // Crop kiri bawah 
		{
...
// Definisikan rasio
const ratioA = 0.4350453172;
const ratioB = 0.8700906344;
const ratioC = 1.3051359517;
const ratioD = 1.7401812689;

// Ambil elemen dari DOM
const widthSlider = document.getElementById('width-slider');
const widthValue = document.getElementById('width-value');
const ratioAOutput = document.getElementById('ratio-a');
const ratioBOutput = document.getElementById('ratio-b');
const ratioCOutput = document.getElementById('ratio-c');
const ratioDOutput = document.getElementById('ratio-d');

// Fungsi untuk menghitung dan menampilkan rasio
function calculateRatios() {
	const width = parseFloat(widthSlider.value);

	// Hitung height berdasarkan rasio
	const heightA = Math.round(width * ratioA);
	const heightB = Math.round(width * ratioB);
	const heightC = Math.round(width * ratioC);
	const heightD = Math.round(width * ratioD);

	// Update output
	widthValue.textContent = width;

	ratioAOutput.textContent = `${width}px x ${heightA}px`;
	ratioBOutput.textContent = `${width}px x ${heightB}px`;
	ratioCOutput.textContent = `${width}px x ${heightC}px`;
	ratioDOutput.textContent = `${width}px x ${heightD}px`;
}

// Event listener untuk slider
widthSlider.addEventListener('input', calculateRatios);

// Hitung rasio awal saat halaman dimuat
calculateRatios();
...
window.addEventListener("scroll", function() {
    ...
    if (currentScroll > lastScrollTop) {
        // Scroll ke bawah
        footer.classList.add("hidden"); // Tambahkan kelas hidden
    } else {
        // Scroll ke atas
        footer.classList.remove("hidden"); // Hapus kelas hidden
    }
